#!/usr/bin/env python3
"""Mesh gateway that authorizes sealed IRON VAULT rack zone requests.
...
authenticated with the VENT node identifier byte 0x07. An authenticated
request is decrypted to recover the requested zone, logged, and
answered with a sealed vent command carrying a monotonic sequence
...
try:
    ...
    raise SystemExit(
        "pip install pyserial to run the VENT gateway"
    ) from exc
...
def _parse_args():
    ...
    parser.add_argument("--log", default="vent_log.csv",
...
def _field_key():
    ...
    -------
    bytes
        Thirty-two byte field key shared with the VENT firmware.
    """
    global _FIELD_KEY
...
def _command_body(seq, zone):
    """Build the signed body of a sealed vent command.
    ...
    """
    head = _le32(seq) + bytes([VENT_COMMAND_OPEN])
    return head + zone.to_bytes(2, "little") + _state_tag(seq)


def _seal_command(seq, zone):
    """Seal a vent command with the field key.
...
def _grant(ser, sender, zone):
    """Answer an authenticated request with a sealed vent command.
    ...
    """
    seq = next(_SEQ)
    print(f"VENT open seq={seq} zone={zone}", flush=True)
...
def _open_request(record):
    """Authenticate a request and recover the guarded VENT zone.
