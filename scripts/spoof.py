#!/usr/bin/env python3
"""Inject a forged IRON VAULT rack zone into a node to teach authentication.
...
command deployed the damper; against the hardened node the same attack fails
...
try:
    ...
    raise SystemExit(
        "pip install pyserial to run the VENT spoof tool"
    ) from exc
...
def _add_forge_args(parser):
    ...
    parser.add_argument("--zone", type=int, default=DEFAULT_ZONE,
                        help="Forged rack zone identifier")
...
def _command_body(seq, zone):
    ...
    """
    head = _le32(seq) + bytes([VENT_COMMAND_CLOSE])
...
def main():
    """Inject the forged IRON VAULT rack zone under the gateway address.
