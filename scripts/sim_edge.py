#!/usr/bin/env python3
"""Simulate a real VENT node from a laptop without a Pico.
...
request and answers with a sealed vent command. Students can validate
...
try:
    ...
    raise SystemExit(
        "pip install pyserial to run the VENT edge simulator"
    ) from exc
...
def _tick(ser, args, seq):
    """Transmit one simulated sealed VENT zone request.
