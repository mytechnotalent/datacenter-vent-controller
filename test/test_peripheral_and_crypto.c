 ...
 *
 * DESCRIPTION:
 * Native unit tests for the RP2350 IRON VAULT datacenter vent
 * controller tower light, manual purge button, vent damper servo,
...
static void assert_lamps(int red, int yellow, int green) {
    ...
    TEST_ASSERT_EQUAL_INT(yellow,
                          mock_gpio_get(VENT_YELLOW_LED_PIN));
    TEST_ASSERT_EQUAL_INT(green,
                          mock_gpio_get(VENT_GREEN_LED_PIN));
}
 ...
 * @return void
 */
static void assert_show(vent_led_state_t state, int red, int yellow,
                        int green) {
...
static void test_status_led_show(void) {
    ...
    assert_show(VENT_OFF, 0, 0, 0);
    assert_show(VENT_HALL_HOT, 1, 0, 0);
    assert_show(VENT_LOCKED, 0, 1, 0);
    assert_show(VENT_COOLING_OK, 0, 0, 1);
}
 ...
 * @return void
 */
static void test_purge_pressed(void) {
    reset_pc();
    TEST_ASSERT_TRUE(purge_init());
    gpio_put(VENT_BUTTON_PIN, true);
    TEST_ASSERT_FALSE(purge_pressed());
    gpio_put(VENT_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(purge_pressed());
}
 ...
 * @return void
 */
static void test_purge_consume(void) {
    reset_pc();
    purge_init();
    gpio_put(VENT_BUTTON_PIN, true);
    TEST_ASSERT_FALSE(purge_consume_press());
    gpio_put(VENT_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(purge_consume_press());
    TEST_ASSERT_FALSE(purge_consume_press());
}
 ...
 * @return void
 */
static void purge_release(void) {
    gpio_put(VENT_BUTTON_PIN, true);
    purge_consume_press();
}
 ...
 * @return void
 */
static void test_purge_debounce(void) {
    reset_pc();
    purge_init();
    gpio_put(VENT_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(purge_consume_press());
 ...
 * @return void
 */
static void test_purge_debounce_elapsed(void) {
    reset_pc();
    purge_init();
    gpio_put(VENT_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(purge_consume_press());
    purge_release();
    mock_timer_set_us(mock_timer_now_us() + VENT_PURGE_DEBOUNCE_US);
    gpio_put(VENT_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(purge_consume_press());
}

/**
 * @brief Verify purge_reset clears the debounce state.
 *
 * @param void No parameters.
 * @return void
 */
static void test_purge_reset(void) {
    reset_pc();
    purge_init();
    gpio_put(VENT_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(purge_consume_press());
...
static void test_servo_init(void) {
    ...
    slice = pwm_gpio_to_slice_num(VENT_SERVO_PIN);
...
static void test_ir_init(void) {
    ...
    TEST_ASSERT_FALSE(s_mock_gpio_dirs[VENT_IR_PIN]);
}
...
static void run_indicator_tests(void) {
    ...
    RUN_TEST(test_purge_pressed);
    RUN_TEST(test_purge_consume);
    RUN_TEST(test_purge_debounce);
    RUN_TEST(test_purge_debounce_elapsed);
    RUN_TEST(test_purge_reset);
}
